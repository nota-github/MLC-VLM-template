...
from ..phi3.phi3_model import Phi3Config, Phi3ForCausalLM

logger = logging.getLogger(__name__)


@dataclasses.dataclass
class PhivaVisionConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    """
...
@dataclasses.dataclass
class PhivaConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    """
    LLaVa Config
    """
    ...
    vision_config: PhivaVisionConfig
    vocab_size: int
    ...
    def __post_init__(self):
        ...
        if isinstance(self.vision_config, PhivaVisionConfig):
        ...
        self.vision_config = PhivaVisionConfig.from_dict(vision_config_dict)
        ...
        self.text_config = Phi3Config.from_dict(text_config_dict)
    ...
    def get_hf_config(self, text_config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            hf_config = MODEL_PRESETS["phiva-4b"]
            # else:
...
class CLIPVisionEmbeddings(Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: PhivaVisionConfig):
        super().__init__()
        self.config = config
    ...
    def forward(self, pixel_values: Tensor) -> Tensor:
        ...
        posi_ids = reshape(wrap_nested(arange(0, self.num_positions, dtype="int32"), name="arange"), shape=(1, -1))
...
class PhivaMultiModalProjector(nn.Module):
    def __init__(self, config: PhivaConfig):
        super().__init__()
...
class PhivaForCasualLM(Module):
    def __init__(self, config: PhivaConfig):
        super().__init__()
        self.config = config
        self.vision_tower = CLIPVisionModel(config.vision_config)
        self.multi_modal_projector = PhivaMultiModalProjector(config)
    ...
    def create_paged_kv_cache(  # pylint: disable=too-many-arguments
        ...
        return PagedKVCache.create_generic(
            ...
            num_attention_heads=self.config.text_config.num_attention_heads // self.config.tensor_parallel_shards,
            num_key_value_heads=self.config.text_config.num_key_value_heads // self.config.tensor_parallel_shards,
    ...
    def get_default_spec(self):
        mod_spec = {
            ...
            "prefill": {
                "input_embed": nn.spec.Tensor([1, "seq_len", self.config.text_config.hidden_size], self.dtype),
            ...
            "decode": {
                "input_embed": nn.spec.Tensor([1, 1, self.config.text_config.hidden_size], self.dtype),
                "paged_kv_cache": nn.spec.Object(object_type=PagedKVCache),
                "$": {
                    "param_mode": "packed",
                    "effect_mode": "none",
            ...
            "batch_prefill": {
                "input_embeds": nn.spec.Tensor([1, "seq_len", self.config.text_config.hidden_size], self.dtype),
            ...
            "batch_decode": {
                "input_embeds": nn.spec.Tensor(["batch_size", 1, self.config.text_config.hidden_size], self.dtype),
                "paged_kv_cache": nn.spec.Object(object_type=PagedKVCache),
                "$": {
                    "param_mode": "packed",
                    "effect_mode": "none",
