...
class Phi3MLP(nn.Module):
    def __init__(self, config: Phi3Config):
        ...
        if config.intermediate_size % config.tensor_parallel_shards != 0:
            raise ValueError(f"Cannot split MLP intermediate size {config.intermediate_size} " f"evenly to {config.tensor_parallel_shards} GPUs.")
...
class PhiMHA(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: Phi3Config):
        ...
        assert config.num_attention_heads % config.tensor_parallel_shards == 0, f"num_attention_heads({config.num_attention_heads}) " "must be divisible by tensor_parallel_shards"
        self.num_key_value_heads = config.num_key_value_heads // config.tensor_parallel_shards
        assert config.num_key_value_heads % config.tensor_parallel_shards == 0, f"num_attention_heads({config.num_key_value_heads}) " "must be divisible by tensor_parallel_shards"
